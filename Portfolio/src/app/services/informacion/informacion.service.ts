...
export class InformacionService {
  ...
  update(informacion: Informacion): Observable<Informacion> {
    return this.http.put<Informacion>(urlRailway + 'informacion/update', informacion);
  }

  updateImg(formData: FormData, type: string): Observable<Imagen> {
    return this.http.post<Imagen>(urlRailway + `imagen/upload/informacion/${type}`, formData);
  }
}
